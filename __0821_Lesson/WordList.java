...
public class WordList {
	...
	public static void main(String[] args) {
		...
		String koreanPart = "";			// 한글 저장 변수. 
		char[] koreanChar = new char[koreanPart.length()];
		for(int i=0;i<koreanPart.length();i++) {
			koreanChar[i] = koreanPart.charAt(i);
		}
		System.out.println(Arrays.toString(koreanChar));
		String sign = "";				// 특문 저장 변수.
		char[] signChar = new char[sign.length()];
		for(int i=0;i<sign.length();i++) {
			signChar[i] = sign.charAt(i);
		}
		System.out.println(Arrays.toString(signChar));
